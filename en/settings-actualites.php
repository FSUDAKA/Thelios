...
$eventNom = $actus->findNomById($event);
$data     = $actus->findAllById($event);
$data1    = $user->findOneById($_SESSION['id']);


if (($_SERVER['REQUEST_METHOD'] == 'POST')){
    if(!empty($_FILES['PIC_ACTUALITE']['name'])){
        $nomOrigine = $_FILES['PIC_ACTUALITE']['name'];
        ...
        if (!(in_array($extensionFichier, $extensionsAutorisees))) {
            ...
            unlink(dirname(__FILE__)."/".$dossierimage."/" . $_POST["OLD_PIC_ACTUALITE"]);

            if (move_uploaded_file($_FILES["PIC_ACTUALITE"]["tmp_name"],$repertoireDestination.$nomDestination)) {
                $actus->setPicActualite($nomDestination, $event);
            }
        }
    }

    $TXT_ACTUALITE = $_POST['TXT_ACTUALITE'];
    $TITRE_ACTAUALITE = $_POST['TITRE_ACTUALITE'];
	$actus->updateTXT($TXT_ACTUALITE, $event);
            ...
            if(!empty($_FILES['PIC_' . $i]['name'])){
                ...
                if (!(in_array($extensionFichier, $extensionsAutorisees))) {
                    ...
                    if (move_uploaded_file($_FILES[$namepicture]["tmp_name"],$repertoireDestination.$nomDestination)) {
                        $actus->setFileBloc($i, $nomDestination, $event);
                    }
                }
            }
            $actus->setTxtBloc($i, $_POST["TITRE_".$i], $_POST["TXT_".$i], $_POST["NB_BLOCS"], $event);
        }
    }
}
					...
					<div class="row uniform">
						...
						<div class="9u 12u$(small)" style="width: 100%">
							...
							<table style="margin-top:30px;">
                                <thead>
                                    <tr>
                                        <th>Page d'actualité</th>
                                        <th style="text-align: right;"><a data-fancybox data-type="iframe" data-src="actualites.php?event=<?php echo $event; ?>&view=admin" href="javascript:;" style="text-decoration:none;"><i class="fas fa-eye"></i></a></th>
                                    </tr>
                                </thead>
                            </table>

                            <form method="post" action="settings-actualites.php?event=<?php echo $event; ?>" enctype="multipart/form-data">
                                <div class="row uniform">
                                    <div class="12u$ 12u$(xsmall)">
                                        <div class="fakeinput">Bannière :
                                            <input id="PIC_ACCUEIL" name="PIC_ACTUALITE" type="file" onchange="displayContent(event,0,0)" class="fake-btn">
                                            ...
                                            <?php if($data["PIC_ACTUALITE"] != ""){
                                                echo'
                                                    <a href="delete.php?picture='.$data['PIC_ACTUALITE'].'&url=settings-actualites.php&event='.$event.'&jour=PIC_ACTUALITE&table=actualite" class="confirm" style="text-decoration:none; float:right; margin-left: 10px;" title="Supprimer">
                                                        <i class="fas fa-trash"></i>
                                                    </a>
                                                    <a data-fancybox="images" href="images/'.$event.'/'.$data["PIC_ACTUALITE"].'" style="text-decoration:none; float:right;" title="Voir la photo">
                                                        <div class="miniature" style="background: url(images/'.$event.'/'.$data["PIC_ACTUALITE"].')"></div>
                                                    </a>';
                                            } ?>
                                        </div>
                                    ...
                                    <div class="12u$ 12u$(xsmall)">
                                        <input type="text" name="TXT_ACTUALITE" id="TXT_ACCUEIL_T_EDITO" value="<?php echo $data['TXT_ACTUALITE']; ?>" placeholder="Sous-titre">
                                    </div>
	                                ...
	                                <?php if(isset($_POST['NB_BLOCS'])){
		                                ...
		                                $sql = "SELECT * FROM `SITE_ACTUS` WHERE EVENT_ID = '$event'";
		                                $req = mysqli_query($db, $sql) or die('Erreur SQL !<br />' . $sql . '<br />' . mysql_error());
		                                $data = mysqli_fetch_assoc($req);
		                                mysqli_free_result($req);
		                                mysqli_close($db);

		                                $NB_BLOCS = $data['NB_ACTUS'];
                                        ...
                                        <div class="row uniform">
                                            ...
                                            <div class="12u$ 12u$(xsmall)">
                                                <div class="fakeinput">Image :
                                                    ...
                                                    <?php if($data[$PIC] != ""){
                                                        echo'
                                                            <a href="delete.php?picture='.$data[$PIC].'&url=settings-actualites.php&event='.$event.'&jour=PIC_'.$i.'&table=actualite" class="confirm" style="text-decoration:none; float:right; margin-left: 10px;" title="Supprimer">
                                                                <i class="fas fa-trash"></i>
                                                            </a>
                                            ...
                                            <div class="12u$">
                                                <textarea name="TXT_<?=$i?>" class="summernote" id="TXT_ACCUEIL_EDITO" placeholder="Edito" rows="6"><?php echo $data['TXT_' . $i]; ?></textarea>
                                            </div>
                                        </div>
                                    </div>
                                    <?php } ?>
